...
**Spoiler**: just add `compile 'io.particle:cloudsdk:0.1.2'` to your `build.gradle`)
...
### Logging
HTTP logging can be configured by setting the `http_log_level` string resource.
Valid values follow Retrofit's [`LogLevel` enum](http://square.github.io/retrofit/javadoc/retrofit/RestAdapter.LogLevel.html) : `NONE`, `BASIC`, `HEADERS`, `HEADERS_AND_ARGS`, or `FULL`
